...
BEGIN
    ...
    IF NEW.BeginDate IS DISTINCT FROM OLD.BeginDate
       ...
       OR NEW.GradingPeriodSequence IS DISTINCT FROM OLD.GradingPeriodSequence
...
CREATE OR REPLACE FUNCTION edfi.StudentLearningObjective_UpdLastModDate()
RETURNS trigger AS
$BODY$
BEGIN
    -- Check if any volatile foreign key values have changed
    IF NEW.BeginDate IS DISTINCT FROM OLD.BeginDate
       ...
       WHERE rt.GradingPeriodDescriptorId = NEW.GradingPeriodDescriptorId
         ...
         AND rt.GradingPeriodSchoolId = NEW.GradingPeriodSchoolId
         AND rt.GradingPeriodSchoolYear = NEW.GradingPeriodSchoolYear
         AND rt.LearningObjectiveId = NEW.LearningObjectiveId
         AND rt.Namespace = NEW.Namespace
         AND rt.StudentUSI = NEW.StudentUSI;
    END IF;
    RETURN NEW;
...
IF NOT EXISTS(SELECT 1 FROM information_schema.triggers WHERE trigger_name = 'updaterootlastmodifieddate' AND event_object_schema = 'edfi' AND event_object_table = 'studentlearningobjectivestudentsectionassociation') THEN
CREATE TRIGGER updaterootlastmodifieddate
  AFTER UPDATE ON edfi.studentlearningobjectivestudentsectionassociation
  FOR EACH ROW
  EXECUTE FUNCTION edfi.StudentLearningObjective_UpdLastModDate();
END IF;
