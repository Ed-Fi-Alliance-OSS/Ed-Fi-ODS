...
BEGIN
    ...
    IF @claimId IS NULL
        BEGIN
            ...
            INSERT INTO dbo.ResourceClaims(DisplayName, ResourceName, ClaimName, ParentResourceClaimId, Application_ApplicationId)
            VALUES ('studentSpecialEducationProgramEligibilityAssociation', 'studentSpecialEducationProgramEligibilityAssociation', 'http://ed-fi.org/ods/identity/claims/studentSpecialEducationProgramEligibilityAssociation', @parentResourceClaimId, @applicationId)
    ...
    PRINT 'Adding authorization strategy ''RelationshipsWithStudentsOnlyThroughResponsibility'' for resource claim ''' + @claimName + ''' (claimId=' + CONVERT(nvarchar, @claimId) + ').'
    INSERT INTO dbo.ResourceClaimActionAuthorizationStrategies(ResourceClaimActionId, AuthorizationStrategyId)
    VALUES (@resourceClaimActionId, @authorizationStrategyId)
    ...
    PRINT 'Adding authorization strategy ''RelationshipsWithStudentsOnlyThroughResponsibilityIncludingDeletes'' for resource claim ''' + @claimName + ''' (claimId=' + CONVERT(nvarchar, @claimId) + ').'
    INSERT INTO dbo.ResourceClaimActionAuthorizationStrategies(ResourceClaimActionId, AuthorizationStrategyId)
    VALUES (@resourceClaimActionId, @authorizationStrategyId)
