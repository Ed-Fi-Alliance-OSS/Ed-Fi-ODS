        ...
        public void Execute(TContext context, TResult result)
        {
            ...
            if (!validationResults.IsValid())
            {
                result.Exception = new ValidationException(
                    $"Validation of '{((object) context.Resource).GetType().Name}' failed.{Environment.NewLine}{string.Join(Environment.NewLine, validationResults.GetAllMessages(indentLevel: 1))}");
            }
        }
