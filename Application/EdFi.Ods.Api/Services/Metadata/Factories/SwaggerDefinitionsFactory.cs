...
namespace EdFi.Ods.Api.Services.Metadata.Factories
{
    public class SwaggerDefinitionsFactory
    {
        ...
        public SwaggerDefinitionsFactory(
            ISwaggerDefinitionsFactoryEntityExtensionStrategy entityExtensionStrategy,
        ...
        private static Schema EtagSchema => new Schema
                                            {
        ...
        public IDictionary<string, Schema> Create(IList<SwaggerResource> swaggerResources)
        {
            ...
            swaggerResources.Where(x => _swaggerFactoryResourceFilterStrategy.ShouldInclude(x.Resource))
                            .Select(
                                 r => new
                                      {
            ...
            swaggerResources.SelectMany(
                                 r =>
                                     r.Resource.AllContainedItemTypes.Where(
                                           x =>
                                               _swaggerFactoryResourceFilterStrategy.ShouldInclude(x))
                                      .Select(
                                           i =>
                                               new
                                               {
                                      ...
                                      .Concat(
                                           swaggerResources.SelectMany(s => s.Resource.AllContainedReferences)
                                                           .Select(
                                                                reference => new
                                                                             {
                                                                                 key =
                                                                                     _swaggerDefinitionsFactoryNamingStrategy.GetReferenceName(
                                                                                         r.Resource,
                                                                                         reference),
            ...
            _definitionsFactoryEntityExtensionStrategy.GetEdFiExtensionBridgeDefinitions(swaggerResources)
                                                      .ForEach(
                                                           pair => definitions.Add(pair.Key, pair.Value));

            return
                new SortedDictionary<string, Schema>(definitions);
        }

        private static Schema RefSchema(string referenceName) => new Schema
                                                                 {
        ...
        private static string ReferenceDescription(
            string referencedResourceName,
            string roleName,
            string otherEntityName) => $@"A reference to the related {referencedResourceName} resource for {roleName?.ToLower()}{otherEntityName}.";

        private Schema CreateResourceChildSchema(ResourceChildItem resourceChildItem, SwaggerResource swaggerResource)
        {
            var properties =
                resourceChildItem.Properties.Select(
                                      p =>
                                          new
                                          {
                                              IsRequired = p.IsIdentifying || !p.PropertyType.IsNullable, Key = UniqueIdSpecification
                                                                                                               .GetUniqueIdPropertyName(
                                                                                                                    p.JsonPropertyName)
                                                                                                               .ToCamelCase(),
                                              Schema = SwaggerDocumentHelper.CreatePropertySchema(p)
                                          })
                                 .Concat(
                                      resourceChildItem.References
                                                       .Select(
                                                            r =>
                                                                new
                                                                {
                                                                    r.IsRequired, Key = r.PropertyName.ToCamelCase(), Schema = new Schema
                                                                                                                               {
                                                                                                                                   @ref =
                                                                                                                                       SwaggerDocumentHelper
                                                                                                                                          .GetDefinitionReference(
                                                                                                                                               _swaggerDefinitionsFactoryNamingStrategy
                                                                                                                                                  .GetReferenceName(
                                                                                                                                                       swaggerResource
                                                                                                                                                          .Resource,
                                                                                                                                                       r))
                                 ...
                                 .Concat(
                                      resourceChildItem.Collections
                                                       .Select(
                                                            c =>
                                                                new
                                                                {
                                                                    IsRequired = c.Association.IsRequiredCollection, Key = c.JsonPropertyName,
                                                                    Schema = CreateCollectionSchema(c, swaggerResource)
                                                                })
                                  )
                                 .Concat(
                                      resourceChildItem.EmbeddedObjects
                                                       .Select(
                                                            e =>
                                                                new
                                                                {
                                                                    IsRequired = false, Key = e.JsonPropertyName,
            ...
            if (bridgeSchema != null)
            {
                properties.Add(
                    new
                    {
                        IsRequired = false, Key = ExtensionCollectionKey, Schema = bridgeSchema
                    });
            }

            var requiredProperties = properties.Where(x => x.IsRequired)
                                               .Select(x => x.Key)
                                               .ToList();

            return new Schema
                   {
                       type = "object", required = requiredProperties.Any()
                           ? requiredProperties
                           : null,
        ...
        private Schema CreateCollectionSchema(Collection collection, SwaggerResource swaggerResource)
        {
            return new Schema
                   {
                       type = "array", description = CollectionDescription(
                           collection.ItemType.PluralName.ToCamelCase(),
                           collection.ItemType.Description.ScrubForOpenApi()),
                       items = RefSchema(_swaggerDefinitionsFactoryNamingStrategy.GetCollectionReferenceName(swaggerResource, collection))
                   };
        }
            ...
            return new Schema
                   {
                       @ref =
                           SwaggerDocumentHelper.GetDefinitionReference(
                               _swaggerDefinitionsFactoryNamingStrategy
                                  .GetEmbeddedObjectReferenceName(swaggerResource, embeddedObject))
                   };
        }
        ...
        private Schema CreateResourceSchema(SwaggerResource swaggerResource)
        {
            ...
            var properties = resource.UnifiedKeyAllProperties()
                                     .Select(
                                          x => new PropertySchemaInfo
                                               {
                                                   PropertyName = x.JsonPropertyName, IsRequired = !x.PropertyType.IsNullable && !x.IsServerAssigned,
                                              ...
                                              x => new PropertySchemaInfo
                                                   {
                                                       ...
                                                       IsRequired = x.Association.IsRequiredCollection, Sort = SortOrder(
                                                           x.PropertyName,
                                                           x.Association.IsRequiredCollection),
                                     ...
                                     .Concat(
                                          resource.EmbeddedObjects.Select(
                                              x => new PropertySchemaInfo
                                                   {
                                                       PropertyName = x.JsonPropertyName, Sort = SortOrder(x.PropertyName, false),
                                     ...
                                     .Concat(
                                          resource.References
                                                  .Select(
                                                       x => new PropertySchemaInfo
                                                            {
                                                                PropertyName = x.PropertyName, IsRequired = x.IsRequired, IsReference = true,
                                                                Sort = 3, Schema = new Schema
                                                                                   {
                                                                                       @ref =
                                                                                           SwaggerDocumentHelper
                                                                                              .GetDefinitionReference(
                                     ...
                                     .Distinct(new PropertySchemaInfoComparer())
                                     .ToList();

            var propertyDict = properties.OrderBy(x => x.Sort)
            ...
            var requiredProperties = properties.Where(
                                                    x =>
                                                        x.IsRequired
                                                        || x.PropertyName.EqualsIgnoreCase("id"))
        ...
        private Schema GetEdFiExtensionBridgeReferenceSchema(ResourceClassBase resource, ISwaggerResourceContext resourceContext)
        {
            ...
            {
                return _definitionsFactoryEdFiExtensionBridgeStrategy.GetEdFiEntityExtensionBridgeSchema(resource, resourceContext);
            }
