    ...
    public class GraphML
    {
        ...
        public static GraphML Create(BidirectionalGraph<EntityWithDataOperation, DataOperationEdge> graph)
        {
            return new GraphML
            {
                ...
                Nodes = graph.Vertices.Where(x => EntitySpecification.HasResourceRepresentation(x.Entity))
                    .Select(
                        x =>
                        {
                            var entity = x.ContextualConcreteEntity ?? x.Entity;

                            return new Node {Id = $"/{entity.SchemaUriSegment()}/{entity.PluralName.ToCamelCase()}"};
                        })
                    .Distinct(Node.NodeComparer)
                    .ToList(),
                Edges = graph.Edges.Where(x => EntitySpecification.HasResourceRepresentation(x.Source.Entity))
                    .Select(
                        x =>
                        {
