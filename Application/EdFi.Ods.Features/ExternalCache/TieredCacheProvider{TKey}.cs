...
public class TieredCacheProvider<TKey> : ICacheProvider<TKey>, IAsyncCacheProvider<TKey>, IClearable
{
    ...
    public bool TryGetCachedObject(TKey key, out object value)
    {
        if (TryGetLocalCacheValue(key, out value))
        {
            ...
            {
                _logger.Debug($"[Hybrid] L1 (in-memory) cache hit for key '{CacheKeyLogSanitizer.SanitizeKeyForLogging(key)}'.");
            }
        ...
        if (_distributedCacheProvider.TryGetCachedObject(key, out value))
        {
            ...
            if (_logger.IsDebugEnabled)
            {
                _logger.Debug($"[Hybrid] L1 miss; L2 (external) cache hit for key '{CacheKeyLogSanitizer.SanitizeKeyForLogging(key)}'; populated L1.");
            }
        ...
        if (_logger.IsDebugEnabled)
        {
            _logger.Debug($"[Hybrid] L1/L2 cache miss for key '{CacheKeyLogSanitizer.SanitizeKeyForLogging(key)}'.");
        }
