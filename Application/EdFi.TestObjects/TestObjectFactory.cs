            ...
            CustomObjectBuildContext customObjectBuildContext)
        {
            ...
            while (writableProperties.Count > 0)
            {
                ...
                foreach (var property in writableProperties.ToArray())
                {
                        ...
                        || propertyValueConstraints.TryGetValue(explicitType.Name + property.Name, out constraintValue))
                    {
                        ...
                        catch (Exception ex)
                        {
                            ...
                            _logger.DebugFormat(
                                "Normal value building will proceed for path '{0}' because the property '{1}' (of type '{2}') could not be set to value '{3}' (of type '{4}') on instance type '{5}' due to the following exception:{6}{7}.",
                                logicalPropertyPath,
                                property.Name,
                ...
                if (writableProperties.Count == lastWritableCount)
                {
                    throw new Exception(
                        string.Format(
                            "The attempts to build the following properties have failed to make progress due to 'Defer' responses from the value builders:{0}{1}.",
                            Environment.NewLine, string.Join(", ", writableProperties.Select(p => p.Name))));
                }
            }
        }
