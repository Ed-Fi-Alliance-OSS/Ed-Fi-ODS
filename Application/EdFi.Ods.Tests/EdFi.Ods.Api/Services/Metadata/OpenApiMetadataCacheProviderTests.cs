...
namespace EdFi.Ods.Tests.EdFi.Ods.Api.Services.Metadata
{
    ...
    public class OpenApiMetadataCacheProviderTests
    {
        ...
        private static List<RouteInformation> GetTestRouteInformation()
        {
            List<RouteInformation> infoList = new List<RouteInformation>();
            foreach (var route in typeof(MetadataRouteConstants).GetProperties())
            {
                RouteInformation info = new RouteInformation();
                info.Name = Convert.ToString(route.GetValue(null));
                info.Template = Convert.ToString(route.GetValue(null));
                infoList.Add(info);
            }
        ...
        public class When_requesting_the_sdk_gen_section_from_the_cache : TestFixtureBase
        {
            ...
            protected override void Arrange()
            {
                ...
                //TODO - Not supporting NETSTANDARD
                ...
                var infoList = GetTestRouteInformation();

                foreach (RouteInformation info in infoList)
                {
                    var OpenAPIMetadataRouteInfo = Stub<IOpenApiMetadataRouteInformation>();
                    A.CallTo(() => OpenAPIMetadataRouteInfo.GetRouteInformation()).Returns(info);
                    _openAPIMetadataRouteInformation.Add(OpenAPIMetadataRouteInfo);
                }

                var _resourceModelProvider = Stub<IResourceModelProvider>();
                var resourcemodeldata = ResourceModelProvider.GetResourceModel();
                A.CallTo(() => _resourceModelProvider.GetResourceModel()).Returns(resourcemodeldata);

                _openApiMetadataCacheProvider = new OpenApiMetadataCacheProvider(_resourceModelProvider, _openAPIMetadataRouteInformation, TestOpenApiContentProviders);
            }
        ...
        public class When_requesting_the_profiles_section_from_the_cache : TestFixtureBase
        {
            ...
            protected override void Arrange()
            {
                //TODO - need to convert to support net core
                //  AssemblyLoader.EnsureLoaded<Marker_EdFi_Ods_Profiles_Test>();
        ...
        public class When_requesting_a_section_from_the_cache_for_which_no_route_was_registered : TestFixtureBase
        {
            ...
            protected override void Arrange()
            {
                ...
                _openApiMetadataCacheProvider = new OpenApiMetadataCacheProvider(_resourceModelProvider, _openAPIMetadataRouteInformation, _openApiContentProviders);
