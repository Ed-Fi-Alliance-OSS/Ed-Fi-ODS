                ...
                Func<ProfileResourceModel, IReadOnlyDictionary<FullName, ProfileResourceContentTypes>> contentTypesByFullName)
            {
                ...
                if (allProfileResources.Any())
                {
                    ...
                    if (allProfileResources.All(x => x == null))
                    {
                        ...
                        throw new ProfileMethodUsageException(_usage, $"There is no {_usage.ToString().ToLower()} content type available to the caller for the '{fullName.Name}' data in the following profiles: '{string.Join("', '", profileNames)}'.");
                    }
