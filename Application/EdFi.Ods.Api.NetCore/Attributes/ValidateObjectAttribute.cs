    ...
    public class ValidateObjectAttribute : ValidationAttribute
    {
        protected override ValidationResult IsValid(object value, ValidationContext validationContext)
        {
            ...
            var compositeResults =
                new CompositeValidationResult(string.Format("Validation of '{0}' failed.", validationContext.DisplayName));
                ...
                foreach (var itemResult in itemResults)
                {
                    ...
                    else
                    {
                        compositeResults.AddResult(
                            new ValidationResult(string.Format("{0}: {1}", context.DisplayName, itemResult)));
                    }
                }
            }
