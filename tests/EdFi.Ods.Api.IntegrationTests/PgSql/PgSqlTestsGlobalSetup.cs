...
{
    [SetUpFixture]
    public class PgSqlTestsGlobalSetup
    {
        ...
        private PgSqlDatabaseHelper _databaseHelper;
        ...
        public void Setup()
        {
            _configuration = new ConfigurationBuilder()
                ...
                .AddJsonFile("appsettings.pgsql.json", optional: true)
                .Build();

            _databaseHelper = new PgSqlDatabaseHelper(_configuration);
            ...
            if (string.IsNullOrWhiteSpace(maintenanceConnectionString))
            {
                throw new ApplicationException(
                    "Missing configuration entry: appsettings.pgsql.json/ConnectionStrings/EdFi_Master");
            }
            ...
            {
                throw new ApplicationException(
                    "Missing configuration entry: appsettings.pgsql.json/ConnectionStrings/EdFi_Ods");
            }
            ...
            if (engines.All(engine => engine != "PostgreSQL"))
            {
                TestDisabledReason = "PostgreSQL isn't present in appsettings.json/Engines";
                return;
            }
            ...
            catch
            {
                ...
                var reason =
                    "Couldn't connect to PostgreSQL server, verify appsettings.pgsql.json/ConnectionStrings/EdFi_Master";
                if (testMode == "Strict")
                {
            ...
            try
            {
                using var conn = new NpgsqlConnection(odsConnectionString);
                conn.Open();
            }
            catch
            {
                ...
                var reason = "Couldn't open template database, verify appsettings.pgsql.json/ConnectionStrings/EdFi_Ods";
                if (testMode == "Strict")
                {
                    TestFailedReason = reason;
                }
            ...
            var connectionStringBuilder = new NpgsqlConnectionStringBuilder { ConnectionString = odsConnectionString };

            _databaseHelper.CopyDatabase(connectionStringBuilder.Database, testDbName);
