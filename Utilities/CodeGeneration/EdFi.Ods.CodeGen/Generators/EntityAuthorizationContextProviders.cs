    ...
    public class EntityAuthorizationContextProviders : GeneratorBase
    {
            ...
            get
            {
                if (_primaryKeysInheritedFromEducationOrganization == null)
                {
                    var educationOrganization =
                        _resourceModel.GetResourceByFullName(new FullName(EdFiConventions.PhysicalSchemaName, EducationOrganizationTableName));
        ...
        protected override object Build()
        {
            var model =
                new
                {
                    ClaimsNamespace = Namespaces.Common.Claims, EntitiesNamespace =
                        EdFiConventions.BuildNamespace(
                            Namespaces.Entities.Common.BaseNamespace,
                            TemplateContext.SchemaProperCaseName),
                    AuthorizationStrategyNamespace = Namespaces.Security.Relationships, AggregateEntityIncludes = GetAggregateEntityIncludes()
                       .Select(
                            i => new
                                 {
                                     Include = i
                    ...
                    ResourcesToRender = _authorizationPropertiesByResource.Select(
                        r => new
                             {
                                 r.Key.Entity.Schema, ResourceName = r.Key.Name, ResourceNameCamelCase = r.Key.Name.ToCamelCase(),
                                 SchemaIsEdFi = r.Key.IsEdFiResource(), AuthorizationResourceProperties = r.Value.OrderBy(p => p.PropertyName)
                                                                                                           .Select(
                                                                                                                p => new
                                                                                                                     {
        ...
        {
            return
                _authorizationPropertiesByResource.Select(r => r.Key.Entity.AggregateNamespace(TemplateContext.SchemaProperCaseName));
        }
        ...
        private IEnumerable<AuthorizationContextProperty> EducationOrganizationProperties(Resource resource)
        {
            ...
            var roleNamedEducationOrganizationColumnsInResource =
                resource.AllProperties
                        .Where(
                             p =>
                                 EducationOrganizationColumns.Any(c => c.PropertyName != p.PropertyName && p.PropertyName.EndsWith(c.PropertyName)))
            ...
            return educationOrganizationColumnsInResource.Take(1)
                                                         .Select(
                                                              p => new AuthorizationContextProperty
                                                                   {
                                                                       PropertyName = p.PropertyName, PropertyType = p.PropertyType.ToCSharp(),
                                                                       IsIdentifying = p.IsIdentifying, IsIncluded = true, Reason =
                                                                      ...
                                                                      p => new AuthorizationContextProperty
                                                                           {
                                                                               ...
                                                                               IsIdentifying = p.IsIdentifying, IsIncluded = false, Reason =
                                                                      ...
                                                                      p => new AuthorizationContextProperty
                                                                           {
                                                                               PropertyName = p.PropertyName,
                                                                               PropertyType = p.PropertyType.ToCSharp(),
                                                                               IsIdentifying = p.IsIdentifying, IsIncluded = false,
