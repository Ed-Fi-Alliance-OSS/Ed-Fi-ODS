...
namespace EdFi.Ods.CodeGen.Helpers
{
    ...
    {
        private readonly IDictionary<string, string> _repositoryByName = new Dictionary<string, string>(StringComparer.InvariantCultureIgnoreCase);
        ...
        public CodeRepositoryHelper(string codeRepositoryPath)
        {
            ...
            var dirList = codeRepositoryPath.Split('\\').ToList();

            var root = string.Join("\\", dirList.TakeWhile(x => !x.EqualsIgnoreCase(CodeRepositoryConventions.EdFiOdsFolderName)));
            ...
            _repositoryByName.Add(CodeRepositoryConventions.Implementation, Path.Combine(root, CodeRepositoryConventions.EdFiOdsImplementationFolderName));
        }
